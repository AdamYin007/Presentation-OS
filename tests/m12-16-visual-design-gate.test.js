...
async function runTests() {
  ...
  assert(
    blackWhite >= 20.9 && blackWhite <= 21.1,
    `Black/white contrast ≈ 21:1 (got ${blackWhite.toFixed(2)})`,
  );
  ...
  assert(
    grayWhite >= 4.4 && grayWhite <= 4.6,
    `Gray/white contrast ≈ 4.5:1 (got ${grayWhite.toFixed(2)})`,
  );
  ...
  assertEqual(
    passResult.verdict,
    "PASS",
    `PASS scenario verdict is PASS (got ${passResult.verdict})`,
  );
  assertEqual(
    passResult.failCount,
    0,
    `PASS scenario has 0 failures (got ${passResult.failCount})`,
  );
  ...
  assertEqual(
    failResult.verdict,
    "FAIL",
    `FAIL scenario verdict is FAIL (got ${failResult.verdict})`,
  );
  ...
  assert(
    failResult.violations.some((v) => v.severity === "fail"),
    "At least one violation has severity='fail'",
  );
  ...
  assertEqual(
    ...
    "NEEDS_REVIEW",
    `NEEDS_REVIEW scenario verdict is NEEDS_REVIEW (got ${reviewResult.verdict})`,
  );
  assertEqual(
    reviewResult.failCount,
    0,
    `NEEDS_REVIEW scenario has 0 failures (got ${reviewResult.failCount})`,
  );
  ...
  const consistentLayouts = [
    {
      index: 1,
      role: "title-slide",
      layoutFamily: "title-slide",
      fontSize: { heading: 44, body: 20 },
    },
    {
      index: 2,
      role: "content",
      layoutFamily: "title-and-bullets",
      fontSize: { heading: 22, body: 14 },
    },
    {
      index: 3,
  ...
  assertEqual(
    typPass.verdict,
    "PASS",
    `Consistent typography verdict is PASS (got ${typPass.verdict})`,
  );
  assertEqual(
    typPass.failCount,
    0,
    `Consistent typography has 0 failures (got ${typPass.failCount})`,
  );
  ...
  assertEqual(
    typFail.verdict,
    "FAIL",
    `Variable typography verdict is FAIL (got ${typFail.verdict})`,
  );
  ...
  assertEqual(
    typReview.verdict,
    "NEEDS_REVIEW",
    `Moderate variance verdict is NEEDS_REVIEW (got ${typReview.verdict})`,
  );
  assertEqual(
    typReview.failCount,
    0,
    `Moderate variance has 0 failures (got ${typReview.failCount})`,
  );
  ...
  assertEqual(
    brandPass.verdict,
    "PASS",
    `Brand PASS scenario verdict is PASS (got ${brandPass.verdict})`,
  );
  ...
  assertEqual(
    brandFail.verdict,
    "FAIL",
    `Missing slides verdict is FAIL (got ${brandFail.verdict})`,
  );
  assert(brandFail.failCount > 0, "Missing slides has failures");
  assert(
    brandFail.issues.some((i) => i.category === "missing_title_slide"),
    "Detects missing title slide",
  );
  assert(
    brandFail.issues.some((i) => i.category === "missing_closing_slide"),
    "Detects missing closing slide",
  );
  ...
  assertEqual(
    fullPassResult.overallVerdict,
    "PASS",
    `Full gate PASS verdict is PASS (got ${fullPassResult.overallVerdict})`,
  );
  assertEqual(
    fullPassResult.summary.qualityScore,
    100,
    `Full gate quality score is 100 (got ${fullPassResult.summary.qualityScore})`,
  );
  assertEqual(
    fullPassResult.remediationSuggestions.length,
    0,
    "No remediation suggestions for PASS",
  );
  ...
  assertEqual(
    fullFailResult.overallVerdict,
    "FAIL",
    `Full gate FAIL verdict is FAIL (got ${fullFailResult.overallVerdict})`,
  );
  assert(
    fullFailResult.summary.qualityScore < 70,
    `Quality score below 70 for FAIL (got ${fullFailResult.summary.qualityScore})`,
  );
  ...
  const m12_15_blockResult = await runVisualDesignGate(brandSpecs, brandPlan, {
    m12_15_verdict: "FAIL",
  });
  assertEqual(
    m12_15_blockResult.overallVerdict,
    "NEEDS_REVIEW",
    `M12.15 FAIL blocks PASS → NEEDS_REVIEW (got ${m12_15_blockResult.overallVerdict})`,
  );
  assert(
    m12_15_blockResult.m12_15_integration.overridden === true,
    "M12.15 integration flagged as overridden",
  );
  ...
  assertEqual(
    noLayoutResult.overallVerdict,
    "PASS",
    `No layout plan degrades gracefully to PASS (got ${noLayoutResult.overallVerdict})`,
  );
  assertEqual(
    noLayoutResult.summary.failCount,
    0,
    `No layout plan has 0 failures (got ${noLayoutResult.summary.failCount})`,
  );
