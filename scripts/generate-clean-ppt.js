#!/usr/bin/env node
/**
 * Generate Clean Image-Based PPT (10 slides only)
 * 
 * This script:
 * 1. Uses template as base
 * 2. Replaces first 10 slides with AI-generated images
 * 3. Removes all other slides
 * 4. Preserves Logo and essential template elements
 */
...
async function main() {
  ...
  console.log("=== Clean Image-Based PPT Generation ===\n");
  ...
  for (let i = 0; i < Math.min(10, slideImages.length); i++) {
    ...
    // Find image references in slide
    const imgMatches = slideXml.match(/r:embed="rId(\d+)"/g) || [];
    const relMatches = relsXml.match(/Target="([^"]+)"/g) || [];
    
    console.log(`  Slide ${slideNum}: ${imgMatches.length} images found`);
  ...
  // Step 7: Remove notes slides (extra)
