#!/usr/bin/env node
/**
 * Generate PPTX with EMBEDDED images (no external dependencies)
 */
...
async function main() {
  ...
  for (let i = 0; i < slides.length && i < slideFiles.length; i++) {
    const slide = slides[i];
    const imagePath = path.join(outputDir, slideFiles[i]);
    ...
    // Set background image from buffer
